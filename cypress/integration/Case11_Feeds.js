...
describe('Test feeds', () => {
  it('User is able to send money', () => {
    cy.readFile('../GoodDAPP/cypress/fixtures/userMnemonicSave.txt').then(mnemonic => {
      ...
      cy.get('[role="button"]').contains(/Cancel link/i).should('be.visible')
      cy.get('[role="button"]').contains(/Share link/i).should('be.visible')
      cy.get('[role="button"]').contains(/Cancel link/i).click()
      //cy.get('[role="button"]').contains(/Ok/i).click()
