...
const SigninScreen = ({ screenProps, navigation, styles, store }) => {
  const asGuest = true
  ...
  const signupGoogle = () => handleSignIn(config.isPhaseZero ? 'google-old' : 'google')
  const signupFacebook = () => handleSignIn('facebook')
  const signupAuth0 = loginType => handleSignIn(loginType === 'email' ? 'auth0-pwdless-email' : 'auth0-pwdless-sms')
