...
const AmountInput = ({ amount, handleAmountChange, styles, error, title, maxLength }: AmountInputProps) => {
  ...
  const inputPressHandler = useOnPress(() => (isMobile ? Keyboard.dismiss() : null), [])

  return (
    <View style={styles.wrapper}>
      <View style={styles.container}>
        {title && <SectionTitle fontWeight="medium">{title}</SectionTitle>}
        <TouchableWithoutFeedback onPress={inputPressHandler} accessible={false} style={styles.section}>
          <InputGoodDollar
