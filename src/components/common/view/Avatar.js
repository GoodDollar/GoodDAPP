 ...
 * @returns {React.Node}
 */
const CustomAvatar = ({ styles, style, source, onPress, size, imageSize, children, uknowStyle, ...avatarProps }) => (
  <TouchableOpacity
    activeOpacity={1}
    disabled={!onPress}
    onPress={onPress}
  ...
  >
    {source ? (
      ...
      <View style={[{ width: size, height: size, backgroundColor: 'rgba(0, 0, 0, 0)' }, uknowStyle]} {...avatarProps}>
        <UnknownProfileSVG />
      </View>
    )}
    {children}
