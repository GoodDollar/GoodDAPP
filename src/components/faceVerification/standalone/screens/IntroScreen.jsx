...
export const WalletDeletedPopupText = ({ styles }) => (
...
const IntroScreen = ({ styles, screenProps, navigation }) => {
  ...
  const { account: externalAccount, isDelta, firstName, isFVFlowReady } = useContext(FVFlowContext)
  ...
  useEffect(() => {
    log.debug({ enrollmentIdentifier, firstName, isFVFlowReady })

    AsyncStorage.setItem('hasStartedFV', 'true')

    if (enrollmentIdentifier && state !== 'pending' && isFVFlowReady) {
      fireEvent(FV_INTRO, { reverify: isReverify })
      checkDisposalState()
    }
  }, [enrollmentIdentifier, isFVFlowReady, navigateTo, isReverify, state, checkDisposalState])
...
const getStylesFromProps = ({ theme }) => ({
  ...
  standaloneSecurityImage: {
    ...Platform.select({
      web: { width: !isMediumDevice ? 260 : 220, height: !isMediumDevice ? 260 : 220 },
