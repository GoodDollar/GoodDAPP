...
import { cropToFace, isQualityImage as isQualityImageDefault, isValidFace as isValidFaceDefault } from './faceQuality'
...
const FaceCapture = ({
  ...
  const handler = useCallback(_throttle(onFacesProxy, 200), [valid, brightness, captured])
  ...
  const renderFace = ({ bounds, faceID, rollAngle, yawAngle }) => {
    ...
    return (
      <View
        key={faceID}
        transform={[{ rotateZ: `${rollAngle.toFixed(0)}deg` }, { rotateY: `${yawAngle.toFixed(0)}deg` }]}
        style={[
          styles.face,
          {
            ...bounds.size,
  ...
  const takePicture = async (camera, face, viewport) => {
    if (camera) {
      try {
        ...
        console.log(Object.keys(data), captured.length, data.exif, data.deviceOrientation, data.pictureOrientation)
        return isValid.ok
      } catch (e) {
  ...
  return (
    <View style={[styles.container, containerStyle]}>
      ...
      <CameraOverlay color={valid ? 'rgba(0,250,50,0.5)' : 'rgba(0,0,0,0.5)'} {...overlayProps} />
      <View style={styles.facesContainer}>
        <Text style={styles.feedBack}>{message}</Text>
        {process.env.NODE_ENV === 'development' && <Text style={styles.feedBack}>{brightness.brightness}</Text>}
      </View>
      {process.env.NODE_ENV === 'development' && face && <View style={styles.facesContainer}>{renderFace(face)}</View>}
