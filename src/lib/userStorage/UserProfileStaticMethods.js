...
const logger = pino.child({ from: 'UserProfileStaticMethods' })

export interface StaticMethods {
  cleanHashedFieldForIndex(field: string, value: string): string;
  isValidValue(field: string, value: string, trusted: boolean): boolean;
  maskField(fieldType: 'email' | 'mobile' | 'phone', value: string): string;
}

export class UserProfileStaticMethods implements StaticMethods {
  /**
  ...
  static isValidValue = (field: string, value: string, trusted: boolean = false) => {
    const cleanValue = UserProfileStaticMethods.cleanHashedFieldForIndex(field, value)
