...
export class GoodWallet {
   ...
   */
  claim(callbacks: PromiEvents, withDivvi = false): Promise<TransactionReceipt> {
    const claimTx = this.UBIContract.methods.claim()
    if (withDivvi && this.networkId === 42220) {
      claimTx.oldEncodeABI = claimTx.encodeABI
      claimTx.encodeABI = function () {
        return this.oldEncodeABI() + GD_DATASUFFIX
      }
    }
    return this.sendTransaction(claimTx, callbacks)
  }
