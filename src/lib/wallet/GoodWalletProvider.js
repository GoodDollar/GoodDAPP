    ...
    async (seedOrWeb3, type: 'SEED' | 'WEB3WALLET') => {
      try {
        ...
        if (enableHDWallet) {
          ...
          if (type === 'SEED') {
            web3 = new HDWalletProvider(wallet.accounts, wallet.wallet.currentProvider.host)
          }
        ...
        if (isLoggedInRouter) {
          ...
          if (loginAndWatch) {
            ...
            wallet.watchEvents(parseInt(lastBlock), toBlock => userProperties.safeSet('lastBlock', parseInt(toBlock)))
