...
async function metaMyStake(web3: Web3, address: string): Promise<MyStake | null> {
  ...
  debug('Reward USDC claimed', CurrencyAmount.fromFractionalAmount(CDAI[chainId], rewardUSDC.claimed.numerator, rewardUSDC.claimed.denominator).toSignificant(6))
  debug('Reward USDC unclaimed', CurrencyAmount.fromFractionalAmount(CDAI[chainId], rewardUSDC.unclaimed.numerator, rewardUSDC.unclaimed.denominator).toSignificant(6))
  ...
  const result = {
    ...
    rewards: {
      reward: rewardG$,
      reward$: {
        claimed: CurrencyAmount.fromFractionalAmount(CDAI[chainId], rewardUSDC.claimed.numerator, rewardUSDC.claimed.denominator),
        unclaimed: CurrencyAmount.fromFractionalAmount(CDAI[chainId], rewardUSDC.unclaimed.numerator, rewardUSDC.unclaimed.denominator),
      },
      GDAO: rewardGDAO
    },
    stake: { amount, amount$ },
...
async function getRewardGDAO(web3: Web3, address: string, account: string): Promise<MyReward> {
  ...
  const {
    0: claimed,
    1: unclaimed
  } = await stakersDistribution.methods.getUserMintedAndPending([address], account).call()
  ...
  async (web3, protocol, token, iToken): Promise<Fraction> => {
    ...
    if (!RR.equalTo(0)) {
      if (protocol === LIQUIDITY_PROTOCOL.COMPOUND) {
        ...
        socialAPY = (supplyRate.multiply(100).add(compSupplyAPY)).divide(RR)
 ...
 * @returns {Promise<Fraction>>}
 */
export const getTokenPriceInUSDC = memoize<(web3: Web3, protocol: LIQUIDITY_PROTOCOL, token: Token) => Promise<Fraction | null>>(
 ...
 * @returns {Promise<Fraction>>}
 */
const getLiquidity = memoize<(web3: Web3, address: string, protocol: LIQUIDITY_PROTOCOL, token: Token) => Promise<Fraction>>(
  async (web3, address, protocol, token) => {
    const simpleStaking = await simpleStakingContract(web3, address)
...
const getYearlyRewardGDAO = memoize<(web3: Web3, address: string) => Promise<CurrencyAmount<Currency>>>(
  async (web3, address) => {
    ...
    return CurrencyAmount.fromFractionalAmount(GDAO[chainId], yearlyRewardGDAO.numerator, yearlyRewardGDAO.denominator)
  }, (_, address) => address
)
...
const getYearlyRewardG$ = memoize<(web3: Web3, address: string) => Promise<CurrencyAmount<Currency>>>(
  async (web3, address) => {
    ...
    const { blockReward } = await contract.methods.rewardsForStakingContract(address).call() as { blockReward: BigNumber }
...
export async function stake(web3: Web3, address: string, amount: number, inInterestToken: boolean = false): Promise<void> {
  ...
  debug('Donation %', percentage.toFixed(0))
  debug('In interest token', inInterestToken)

  await ERC20Contract(web3, tokenAddress).methods.approve(address, tokenRawAmount).send({ from: account })
  await contract.methods.stake(tokenRawAmount, percentage.toFixed(0), inInterestToken).send({ from: account })
}
...
export async function withdraw(web3: Web3, address: string, percent: number): Promise<void> {
  ...
  await contract.methods.withdrawStake(rawAmount.multiply(amount.decimalScale).toFixed(0), false).send({ from: account })
}
